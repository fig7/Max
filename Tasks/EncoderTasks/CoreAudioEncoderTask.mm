...
- (void) writeTags
{
	...
	// Use (unimplemented as of 10.4.3) CoreAudio metadata functions
		...
		@try {
			...
			NSAssert2(noErr == err, NSLocalizedStringFromTable(@"The call to %@ failed (%@).", @"Exceptions", @""), @"AudioFileOpenURL", GetOSStatusError(err));
		...
		@finally {
			...
			NSAssert2(noErr == err, NSLocalizedStringFromTable(@"The call to %@ failed (%@).", @"Exceptions", @""), @"AudioFileClose", GetOSStatusError(err));
		}
	}	
}
