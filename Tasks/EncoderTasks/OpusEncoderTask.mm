...
#import "OpusEncoderTask.h"
#import "OggOpusEncoder.h"

#include <taglib/opusfile.h>		// TagLib::Ogg::Opus::File
...
@implementation OpusEncoderTask

- (id) init
{
	if((self = [super init])) {
		_encoderClass = [OggOpusEncoder class];
		return self;
	}
	return nil;
}
...
- (void) writeTags
{
	...
	TagLib::Ogg::Opus::File					f						([[self outputFilename] fileSystemRepresentation], false);
	...
	if(nil != album)
		f.tag()->addField([AudioMetadata customizeOpusTag:@"ALBUM"], TagLib::String([album UTF8String], TagLib::String::UTF8));
	...
	if(nil != artist)
		f.tag()->addField([AudioMetadata customizeOpusTag:@"ARTIST"], TagLib::String([artist UTF8String], TagLib::String::UTF8));
	...
	if(nil != composer)
		f.tag()->addField([AudioMetadata customizeOpusTag:@"COMPOSER"], TagLib::String([composer UTF8String], TagLib::String::UTF8));
	...
	if(nil != genre)
		f.tag()->addField([AudioMetadata customizeOpusTag:@"GENRE"], TagLib::String([genre UTF8String], TagLib::String::UTF8));
	...
	if(nil != year)
		f.tag()->addField([AudioMetadata customizeOpusTag:@"DATE"], TagLib::String([year UTF8String], TagLib::String::UTF8));
	...
	if(nil != comment)
		f.tag()->addField([AudioMetadata customizeOpusTag:@"DESCRIPTION"], TagLib::String([comment UTF8String], TagLib::String::UTF8));

	// Track title
	title = [metadata trackTitle];
	if(nil != title)
		f.tag()->addField([AudioMetadata customizeOpusTag:@"TITLE"], TagLib::String([title UTF8String], TagLib::String::UTF8));

	// Track number
	trackNumber = [metadata trackNumber];
	if(nil != trackNumber)
		f.tag()->addField([AudioMetadata customizeOpusTag:@"TRACKNUMBER"], TagLib::String([[trackNumber stringValue] UTF8String], TagLib::String::UTF8));

	// Track total
	trackTotal = [metadata trackTotal];
	if(nil != trackTotal)
		f.tag()->addField([AudioMetadata customizeOpusTag:@"TRACKTOTAL"], TagLib::String([[trackTotal stringValue] UTF8String], TagLib::String::UTF8));

	// Disc number
	discNumber = [metadata discNumber];
	if(nil != discNumber)
		f.tag()->addField([AudioMetadata customizeOpusTag:@"DISCNUMBER"], TagLib::String([[discNumber stringValue] UTF8String], TagLib::String::UTF8));

	// Discs in set
	discTotal = [metadata discTotal];
	if(nil != discTotal)
		f.tag()->addField([AudioMetadata customizeOpusTag:@"DISCTOTAL"], TagLib::String([[discTotal stringValue] UTF8String], TagLib::String::UTF8));

	// Compilation
	compilation = [metadata compilation];
	if(nil != compilation)
		f.tag()->addField([AudioMetadata customizeOpusTag:@"COMPILATION"], TagLib::String([[compilation stringValue] UTF8String], TagLib::String::UTF8));

	// ISRC
	isrc = [metadata ISRC];
	if(nil != isrc)
		f.tag()->addField([AudioMetadata customizeOpusTag:@"ISRC"], TagLib::String([isrc UTF8String], TagLib::String::UTF8));

	// MCN
	mcn = [metadata MCN];
	if(nil != mcn)
		f.tag()->addField([AudioMetadata customizeOpusTag:@"MCN"], TagLib::String([mcn UTF8String], TagLib::String::UTF8));
...
- (NSString *)		fileExtension					{ return @"opus"; }
- (NSString *)		outputFormatName				{ return NSLocalizedStringFromTable(@"Opus", @"General", @""); }
